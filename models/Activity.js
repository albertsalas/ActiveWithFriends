...
Activity.findUserEvents = (user_id , result) => {
    query("SELECT JoinedActivities.activityID, Activity.hostID, Activity.title, Activity.description, DATE_FORMAT(Activity.time, '%h:%i %p - %d %b %Y') as time FROM JoinedActivities INNER JOIN Activity ON JoinedActivities.activityID = Activity.id WHERE JoinedActivities.userID = ?", user_id, (error, res) => {
        result(error, res);
    });
}
